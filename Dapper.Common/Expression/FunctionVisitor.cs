...
namespace Dapper.Common
{
    ...
    /// 数据库表达式构建
    /// </summary>
    internal class FunctionVisitor<T> : ExpressionVisitor
    {
        #region Props
        /// <summary>
        /// 表达式参数
        ...
        internal string Build(Dictionary<string, object> param, Expression expression, bool alias = true)
        {
            ...
            Alias = alias;
            Visit(expression);
            var expressionString = FunExpression.ToString();
            if (alias&&!expressionString.Contains(",")&&Mapper.GetColumn<T>(a=>a.ColumnName==expressionString)!=null)
            {
                expressionString = string.Format("{0} AS {1}",expressionString, Mapper.GetColumn<T>(a => a.ColumnName == expressionString).PropertyName);
            }
        ...
        protected override Expression VisitMethodCall(MethodCallExpression node)
        {
            if (node.Method.DeclaringType == typeof(ConditionType))
            {
                if (node.Arguments.Count == 3 && node.Method.Name.Contains("Between"))
                {
                    ...
                    Operator = ConditionType.GetOperator(node.Method.Name);
        ...
        protected override Expression VisitBinary(BinaryExpression node)
        {
            FunExpression.Append("(");
            Visit(node.Left);
            Operator = ConditionType.GetOperator(node.NodeType);
            FunExpression.AppendFormat(" {0} ", Operator);
            Visit(node.Right);
            FunExpression.Append(")");
            return node;
        ...
        protected override Expression VisitUnary(UnaryExpression node)
        {
            ...
            {
                FunExpression.Append(ConditionType.GetOperator(node.NodeType));
                Visit(node.Operand);
            }
            else
            {
        ...
        internal static string GetColumn(Expression expression)
        {
            ...
            return Mapper.GetColumn<T>(a=>a.PropertyName==propertyName).ColumnName;
        }
        #endregion
