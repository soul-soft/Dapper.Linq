...
namespace Dapper.Common
{
    internal class MysqlFrom<T> : IFrom<T> where T : class, new()
    {
        ...
        {
            var column = WhereVisitor<T>.GetColumn(expression.Body);
            Set(column, value);
            return this;
        }
        /// <summary>
        ...
        {
            _set.AppendFormat("{0}{1} = {2}", _set.Length == 0 ? "" : ",", WhereVisitor<T>.GetColumn(column.Body), new FunVisitor<T>().Build(ref Param, value,false));
            return this;
        }
        /// <summary>
        ...
        /// <param name="size"></param>
        /// <returns></returns>
        public IFrom<T> Take(int index, int size)
        {
