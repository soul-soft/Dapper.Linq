...
namespace Dapper.Common
{
    internal class MysqlFrom<T> : IFrom<T> where T : class, new()
    {
        ...
        {
            columns = columns == "*" ? string.Join(",", Mapper.GetColumnList<T>().Select(s => string.Format("{0} AS {1}", s.ColumnName, s.PropertyName))) : columns;
            QuerySql.AppendFormat("SELECT {0}{1} FROM {2}", _distinct != null ? _distinct + " " : "", columns, FromSql);
            if (_where.Length > 0)
            {
        ...
        {
            var data = Select<TReturn>(new FunctionVisitor<T>().Build(Param, expression, true));
            return data;
        }
        ...
        {
            var data = SelectAsync<TReturn>(new FunctionVisitor<T>().Build(Param, expression, true));
            return data;
        }
        /// <summary>
        /// 查询单个数据
        ...
        {
            var data = Single<TReturn>(new FunctionVisitor<T>().Build(Param, expression, true));
            return data;
        }
        /// <summary>
        /// 异步查询单个实体:指定查询列
        ...
        {
            var task = SingleAsync<TReturn>(new FunctionVisitor<T>().Build(Param, expression, true));
            return task;
        }
        #endregion
        ...
        {
            var colums = Mapper.GetColumnList<T>();
            InsertSql.AppendFormat("INSERT INTO {0} ({1}) VALUES ({2})", FromSql, string.Join(",", colums.Select(s => s.ColumnName)),
                string.Join(",", colums.Select(s => s.PropertyName).Select(c => c = '@' + c).ToArray()));
        ...
        private string UpdateBuild(bool condition)
        {
            ...
            {
                var colums = Mapper.GetColumnList<T>();
                UpdateSql.AppendFormat("UPDATE {0} SET {1}", FromSql, string.Join(",", colums.FindAll(f => f.ColumnKey != ColumnKey.Primary).Select(s => s.ColumnName + " = @" + s.PropertyName)));
            }
            if (condition)
            {
                var column = Mapper.GetColumn<T>(f => f.ColumnKey == ColumnKey.Primary);
                UpdateSql.AppendFormat(" WHERE {0} = @{1}", column.ColumnName, column.PropertyName);
            }
        ...
        private string DeleteBuild(bool condition)
        {
            ...
            {
                var column = Mapper.GetColumn<T>(c => c.ColumnKey == ColumnKey.Primary);
                DeleteSql.AppendFormat(" WHERE {0} = @{1}", column.ColumnName, column.PropertyName);
            }
        ...
        /// <returns></returns>
        public IFrom<T> Where(Queryable<T> query)
        {
            if (query.Count > 0)
            {
                _where.Append(new ConditionVisitor<T>().Build(Param, query.Expressions));
            }
            return this;
        }
        /// <summary>
        ...
        {
            Where(new Queryable<T>(expression));
            return this;
        }
        /// <summary>
        /// Where
        ...
        {
            GroupBy(new FunctionVisitor<T>().Build(Param, groupby, false));
            return this;
        }
        ...
        /// <returns></returns>
        public IFrom<T> Having(Queryable<T> having)
        {
            if (having.Count > 0)
            {
                _having.Append(new ConditionVisitor<T>().Build(Param, having.Expressions));
            }
            return this;
        }
        /// <summary>
        ...
        {
            Having(new Queryable<T>(expression));
            return this;
        }
        #endregion        
        ...
        {
            var name = new FunctionVisitor<T>().Build(Param, orderBy, false);
