...
namespace Dapper.Common
{
    ...
    public class MySqlQuery<T1, T2, T3> : IQueryable<T1, T2, T3> where T1 : class where T2 : class where T3 : class
    {
        ...
        public IQueryable<T1, T2, T3> OrderByDescending<TResult>(Expression<Func<T1, T2, T3, TResult>> expression, bool condition = true)
        {
            if (condition)
            {
                OrderBy(string.Join(",", ExpressionUtil.BuildColumns(expression, _param, _prefix, false).Select(s => string.Format("{0} DESC", s.Value))), condition);
            }
            return this;
        }
    ...
    public class MySqlQuery<T1, T2, T3, T4> : IQueryable<T1, T2, T3, T4> where T1 : class where T2 : class where T3 : class where T4 : class
    {
        #region constructor
        public IDbContext _dbcontext { get; }
        public string _prefix { get; }
